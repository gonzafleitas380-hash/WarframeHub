      ...
      <div className="wf-sb__card--wrap">
        ...
        <table className="wf-sb__table--info">
          <tbody>
            ...
            <tr><th>Polaridades</th><td className="wf-sb__td--blue">{info.polaridades.join(' · ')}</td></tr>
            ...
            <tr><th>Velocidad</th><td>{info.velocidad.toFixed(2)}</td></tr>
            <tr><th>Playstyle</th><td>{info.playstyle}</td></tr>
            <tr><th>Hab. del Helminto</th><td>{info.helminto}</td></tr>
            <tr><th>Elem. Progenitor</th><td>{data.danio[0].emoji} {info.progenitor}</td></tr>
          </tbody>
        </table>
        ...
        <div className="wf-sb__stats--bars">
          ...
          {statKeys.map((key, i) => {
            const s = stats[key];
            const diffEscudo   = s.prime - s.base;
            return (
              <div key={key} className="wf-sb__stat--group">
                ...
                <div className="wf-sb__stat--row">
                  ...
                  <span className={`wf-sb__diff--badge ${diffEscudo === 0 ? 'wf-sb__diff--none' : ''}`}>
                    {diffEscudo === 0 ? '—' : `+${diffEscudo}`}
                  </span>
                </div>
              </div>
            );
